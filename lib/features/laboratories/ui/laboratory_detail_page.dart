...
/// Laboratory detail screen — design only for now (no Cubit / API wiring
/// yet). Mirrors `LaboratoryDto`: contact info plus user/doctor/case counts.
class LaboratoryDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final isActive = laboratory['isActive'] as bool;

    return Scaffold(
      backgroundColor: AppColorsManger.background,
      appBar: AppBar(
        title: Text('تفاصيل المخبر', style: AppTextStyles.font18MediumText),
        actions: [
          IconButton(
            onPressed: () => context.push(Routes.laboratoryFormScreen, extra: laboratory),
                        ...
                        children: [
                          ...
                          Expanded(
                            child: _StatTile(
                              ...
                              label: 'الأطباء',
                              value: '${laboratory['doctorCount'] ?? 0}',
                            ),
                          ),
                          const SizedBox(width: 8),
                          Expanded(
                        ...
                        child: Column(
                          children: [
                            DetailInfoRowWidget(
                              ...
                              label: 'العنوان',
                              value: laboratory['address'] as String? ?? '',
                            ),
                            const Divider(height: 1, color: AppColorsManger.divider),
                            DetailInfoRowWidget(
                              icon: Icons.phone_outlined,
                              label: 'رقم الهاتف',
                              value: laboratory['phoneNumber'] as String? ?? '',
                            ),
                          ],
                        ),
                      ),
