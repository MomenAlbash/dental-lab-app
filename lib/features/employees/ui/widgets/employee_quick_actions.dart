...
/// Contact actions for an employee — call, WhatsApp, map. Mirrors
/// [DoctorQuickActions] minus the email tile (employees have no email field).
///
/// The phone number and address used to be plain label/value rows the user
/// had to copy by hand. Here the same data is the control: each tile
/// launches the matching intent, and tiles whose data is missing render
/// disabled rather than disappearing, so the row's shape stays stable.
class EmployeeQuickActions extends StatelessWidget {
  const EmployeeQuickActions({super.key, required this.employee});
